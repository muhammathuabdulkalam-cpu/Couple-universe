...
export const MusicSearchTab: React.FC<MusicSearchTabProps> = ({ onOpenDedicateModal }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-2xl p-3 shadow-md space-y-2 border ${
        theme === 'light'
        ...
        <div className="relative">
          <Search className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${
            theme === 'light' ? 'text-blue-500' : 'text-rose-400'
          }`} />
          <input
            type="text"
            value={query}
            ...
            className={`w-full border rounded-xl pl-10 pr-9 py-2 text-xs sm:text-sm transition ${
              theme === 'light'
                ? 'bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500'
                : 'bg-slate-800/90 border-white/10 text-white placeholder-slate-400 focus:ring-1 focus:ring-rose-500 focus:border-rose-500'
            }`}
          />
          {query && (
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-500 dark:hover:text-white"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      {searchTerm && (
        <div className="flex items-center justify-between px-1 text-xs text-slate-500 dark:text-slate-300">
          <span className={`font-bold ${theme === 'light' ? 'text-slate-800' : 'text-white'}`}>
            Results for <span className={theme === 'light' ? 'text-blue-600' : 'text-rose-400'}>"{searchTerm}"</span>
          </span>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {[...Array(4)].map((_, i) => (
            <div key={i} className={`border rounded-xl p-3 animate-pulse flex items-center gap-3 ${
              theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/40 border border-white/5'
            }`}>
              <div className={`w-10 h-10 rounded-lg shrink-0 ${theme === 'light' ? 'bg-slate-200' : 'bg-white/10'}`} />
              <div className="flex-1 space-y-1.5">
                <div className={`h-3.5 rounded w-3/4 ${theme === 'light' ? 'bg-slate-200' : 'bg-white/10'}`} />
                <div className={`h-2.5 rounded w-1/2 ${theme === 'light' ? 'bg-slate-100' : 'bg-white/5'}`} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`text-center py-10 px-4 rounded-2xl border ${
          ...
          <Music2 className={`w-8 h-8 mx-auto mb-2 ${theme === 'light' ? 'text-blue-500/60' : 'text-rose-400/60'}`} />
          <p className={`text-xs font-bold ${theme === 'light' ? 'text-slate-700' : 'text-slate-300'}`}>
            {!searchTerm ? 'Type a song title above to search' : 'No matching songs found'}
          </p>
          <p className={`text-[10px] ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'} mt-1`}>
          ...
          {results.map((song) => {
            ...
            return (
              <div
                ...
                className={`group rounded-xl p-2.5 transition flex items-center justify-between gap-2 border shadow-sm ${
                  theme === 'light'
                    ? 'bg-slate-50 hover:bg-slate-100/70 border-slate-200 hover:border-blue-500/30 text-slate-800'
                    : 'bg-slate-900/60 hover:bg-slate-800/80 border-white/10 hover:border-rose-500/30 text-slate-200'
                }`}
              >
                {/* Artwork & Details */}
                <div
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <h4 className={`font-bold text-xs truncate transition ${
                      theme === 'light'
                    ...
                    <p className={`text-[10px] truncate ${theme === 'light' ? 'text-slate-550' : 'text-slate-400'}`}>{song.artist}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className={`p-1 rounded-lg transition ${
                      theme === 'light'
                        ? 'hover:bg-slate-205/60 text-slate-500 hover:text-blue-600'
                  ...
                  <button
                    ...
                    className={`p-1 rounded-lg transition ${
                      theme === 'light' ? 'hover:bg-slate-205/60 text-slate-500 hover:text-slate-800' : 'hover:bg-white/10 text-slate-400 hover:text-white'
                    }`}
                  ...
                  {onOpenDedicateModal && (
                    <button
                      ...
                      className={`p-1 rounded-lg text-[10px] font-semibold border transition ${
                        theme === 'light'
      ...
      {selectedSongForPlaylist && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className={`border rounded-2xl p-5 w-full max-w-md shadow-2xl space-y-3 ${
            theme === 'light' ? 'bg-white border-slate-200 text-slate-800' : 'bg-slate-900 border border-white/10 text-white'
          }`}>
            <div className={`flex items-center justify-between pb-2 border-b ${
              ...
              <button
                ...
                className={`p-1 rounded-full ${
                  theme === 'light' ? 'hover:bg-slate-100 text-slate-500' : 'hover:bg-white/10 text-slate-400 hover:text-white'
                }`}
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className={`flex items-center gap-3 rounded-xl p-2.5 ${theme === 'light' ? 'bg-slate-100' : 'bg-white/5'}`}>
              <img
              ...
              <div className="min-w-0">
                <p className={`font-semibold text-xs truncate ${theme === 'light' ? 'text-slate-900' : ''}`}>{selectedSongForPlaylist.title}</p>
                <p className={`text-[10px] truncate ${theme === 'light' ? 'text-slate-550' : 'text-slate-400'}`}>{selectedSongForPlaylist.artist}</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
              {playlists.map((pl) => (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl border flex items-center justify-between transition group text-xs ${
                    theme === 'light'
                ...
                >
                  <div>
                    <p className={`font-semibold transition ${
                      theme === 'light' ? 'text-slate-700 group-hover:text-blue-600' : 'text-slate-200 group-hover:text-rose-300'
                    }`}>{pl.title}</p>
                    <p className="text-[10px] text-slate-500">{pl.songCount} tracks</p>
                  </div>
                  <Plus className={`w-3.5 h-3.5 transition ${
                    theme === 'light' ? 'text-slate-400 group-hover:text-blue-600' : 'text-slate-400 group-hover:text-rose-400'
                  }`} />
                </button>
              ))}
            </div>
