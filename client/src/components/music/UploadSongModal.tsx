      ...
      <div className="bg-slate-900 border border-white/10 rounded-3xl p-6 w-full max-w-lg text-white shadow-2xl space-y-5">
        ...
        {hasExtractedTags ? (
          <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center gap-2">
            ...
            <span>Successfully extracted ID3 title, artist, album & cover artwork!</span>
          </div>
        ) : (
          <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs font-medium flex items-center gap-2">
            ...
            <span>Song title, artist, album & cover artwork are automatically extracted from your audio file!</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative border-2 border-dashed border-white/15 hover:border-rose-500/50 rounded-2xl p-4 text-center cursor-pointer transition bg-white/5">
              ...
              {isExtracting ? (
                <div className="text-xs font-semibold text-rose-300 flex items-center justify-center gap-2">
                  <Loader2 className="w-4 h-4 text-rose-400 animate-spin" />
                  <span>Extracting ID3 Metadata & Artwork...</span>
                </div>
              ) : audioFile ? (
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 mb-1.5">
              Cover Artwork (Embedded Auto-Extracted or Custom Image)
            </label>

            {coverPreviewUrl ? (
              <div className="flex items-center gap-3.5 p-2.5 bg-white/5 border border-white/10 rounded-2xl">
                <img
                  src={coverPreviewUrl}
                  alt="Cover Preview"
                  className="w-14 h-14 rounded-xl object-cover border border-white/15 shadow-md shrink-0"
                />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-rose-400 font-medium flex items-center gap-1 mt-0.5">
                    ...
                    <span>Artwork ready to be saved</span>
                  </p>
                </div>
                <label className="px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-xs text-white font-semibold cursor-pointer shrink-0 transition">
                  Change Image
                  <input
                    type="file"
                    accept="image/*"
              ...
              <div className="relative border border-white/10 hover:border-rose-500/40 rounded-xl p-3 text-center cursor-pointer transition bg-white/5 flex items-center gap-3">
                ...
                <span className="text-xs text-slate-400 truncate">
                  Select custom album cover (or use embedded ID3 cover)
                </span>
              </div>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Album Name</label>
            <input
              type="text"
              value={album}
